...
class _AddwalletscreenState extends State<Addwalletscreen> {
  ...
  void _saveNewCurrency(BuildContext context) async {
    if(_controllerName.text.length <= 0) {
      ...
      Wallet newWal = Wallet(name: _controllerName.text, amount: double.parse(_inputAmount), currency: _selectCurerncy, note: _controllerNote.text);
      print("New wallet: ${newWal.name} - ${newWal.amount} - ${newWal.currency.name} - ${newWal.note}");
